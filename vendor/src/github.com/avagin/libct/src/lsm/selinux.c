...
static char *selinux_process_label_get(pid_t pid)
{
	...
	if (getpidcon_raw(pid, &ctx) < 0) {
		pr_err("failed to get SELinux context for pid %d", pid);
		return NULL;
	}
 ...
 */
static int selinux_process_label_set(const char *inlabel,
				     int use_default, int on_exec)
{
	...
	if (on_exec) {
		if (setexeccon_raw((char *)label) < 0) {
			pr_err("failed to set new SELinux exec context %s", label);
			return -1;
		}
	} else {
		if (setcon_raw((char *)label) < 0) {
			pr_err("failed to set new SELinux context %s", label);
			return -1;
		}
	}

	pr_info("changed SELinux%s context to %s", on_exec ? " exec" : "", label);
	return 0;
}
