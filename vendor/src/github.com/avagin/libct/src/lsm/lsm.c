...
{
	if (drv) {
		pr_msg("LSM security driver %s", drv->name);
		return;
	}
	...
	pr_msg("Initialized LSM security driver %s", drv->name);
}
...
{
	if (!drv) {
		pr_err("LSM driver not inited");
		return NULL;
	}
	return drv->process_label_get(pid);
}

int lsm_process_label_set(const char *label,
