...
static char *apparmor_process_label_get(pid_t pid)
{
	...
	if (ret < 0 || ret >= 100) {
		pr_err("path name too long");
		return NULL;
	}
again:
	f = fopen(path, "r");
	if (!f) {
		pr_err("opening %s", path);
		if (buf)
			free(buf);
		return NULL;
	}
 ...
 */
static int apparmor_process_label_set(const char *inlabel,
				      int use_default, int on_exec)
{
	...
	if (!check_mount_feature_enabled() && strcmp(label, "unconfined") != 0) {
		pr_warn("Incomplete AppArmor support in your kernel");
		return -1;
	}


	if (strcmp(label, "unconfined") == 0 && apparmor_am_unconfined()) {
		pr_info("apparmor profile unchanged");
		return 0;
	}

	if (aa_change_profile(label) < 0) {
		pr_err("failed to change apparmor profile to %s", label);
		return -1;
	}

	pr_info("changed apparmor profile to %s", label);
	return 0;
}
