...
func Apply(c *cgroups.Cgroup, pid int) (cgroups.ActiveCgroup, error) {
	...
	if !c.AllowAllDevices {
		if err := createAndEnterCgroup(c, pid); err != nil {
			return nil, err
		}
		if err := UpdateAllowedDevices(c, pid); err != nil {
			return nil, err
		}
	}
...
func createAndEnterCgroup(c *cgroups.Cgroup, pid int) error {
	dir, err := cgroups.GetContainerPath("devices", c.Parent, c.Name)
	if err != nil {
		return err
	}
...
func UpdateAllowedDevices(c *cgroups.Cgroup, pid int) error {
	// Atm we can't use the systemd device support because of support for wildcards to allow mknod on any device
	//
